...
function uploadStatusButton(inputId, attribute) {
    var config = getConfig(attribute);
    var uploadButton = $(document).find('#' + inputId).parent();

    uploadButton.find("span").text(config.translations['upload-in-progress']);
    if (uploadButton.hasClass('btn-success')) {
        uploadButton.removeClass('btn-success').addClass('btn-default');
    }
    $(document).find('#' + config.tempAttributeId).attr('disabled', true);
